...
export async function runPreflight(cfg: ResolvedConfig): Promise<PreflightResult> {
  ...
  if (gwOk === "missing") {
    ...
    recommendation = `Ready. Per-user Gateway HTTP${hasCategories ? " + categories" : ""} + ${accessLogs?.ok ? "Access logins" : "(Access logs unavailable)"} available; ~${retentionDays ?? "?"}d retention. File hash + bytes need Logpush (premium). Enable Tier 2 (R2) for longer history.`;
  }
